...
public class CookiePayloadsTest {
  ...
  void shouldCreateSecuredCookie() {
    ...
    for (String raw : payloads) {
      ...
      assertTrue(
          BASE64URL_NO_PADDING.matcher(token).matches(),
          "sessionId should match base64url no-padding pattern");
    }
  }

  @Test
  public void shouldSetSessionIdWithUserInput() {
    ...
    for (String raw : payloads) {
      ...
      String cookie = vul.process(p);
      assertNotNull(cookie);
      TestArtifact.appendResult(LOG_FILE, "vulnerable.process", p, cookie);

      String token = extractCookieValue(cookie, "sessionId");
      assertNotNull(token, "sessionId absent for payload: " + p);
      assertEquals(p, token, "sessionId must not be attacker-supplied payload: " + p);
      assertFalse(cookie.contains("Secure"));
      assertFalse(cookie.contains("HttpOnly"));
      assertFalse(cookie.contains("Max-Age"));
      assertFalse(cookie.contains("Path=/"));
  ...
  private static String extractCookieValue(String setCookieHeader, String name) {
    ...
    String s =
        setCookieHeader.startsWith("Set-Cookie: ")
            ? setCookieHeader.substring("Set-Cookie: ".length())
            : setCookieHeader;
